...
#if defined(NDEBUG)
# error "Glancoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa1940a588ac8c26f9170c788bf1716851d1e36deb5a5e8a836a8757db4173411");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Glancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Glancoin Signed Message:\n";
...
{
    // Glancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60; // Glancoin: 1 minute
static const int64 nTargetSpacing = 1 * 60; // Glancoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Glancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Glancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa1940a588ac8c26f9170c788bf1716851d1e36deb5a5e8a836a8757db4173411");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 31/Dec/2017 Rouhani Urges Calm in Iran as Protests Continue";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041783210fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514686936;
        ...
        assert(block.hashMerkleRoot == uint256("0x53fc5a0ae43c57fb2f9f1e5d6535e37015da00fd6f36ec3484713c94fc23d779"));
...
unsigned char pchMessageStart[4] = { 0xd2, 0xe2, 0xd4, 0xb3 }; // Glancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GlancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GlancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GlancoinMiner(CWallet *pwallet)
{
    printf("GlancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("glancoin-miner");
    ...
    try { loop {
        ...
        printf("Running GlancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GlancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GlancoinMiner, pwallet));
}
